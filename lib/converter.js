...
var GuidePathConverter = require('./GuidePathConverter');
var fs                 = require('fs');
...
TFCRatioConverter.prototype.convert = function() {
    ...
    for ( ; i < size; ++i ) {
        ...
        if ( src !== "" ) {
            ...
            // guidePath
            if ( /guide:\{path:\[(.+?)\]/.test(src) ) {
                ++times;
                // console.log('line: ' + this.line + ' converting guidePath.');
                src = src.replace(/guide:\{path:\[(.+?)\]/g, function(match, args) {
                    return match.replace(args, GuidePathConverter.execute(args, rate));
                });
            }
        }
